...
class _AdminPendudukScreenState extends State<AdminPendudukScreen>
  ...
  Future<void> _saveData() async {
    try {
      ...
      await prefs.setString(
          ...
          json.encode(
              ageDistributionData.map((k, v) => MapEntry(k.toString(), v))));
      await prefs.setString(
          'district_density_data',
          json.encode(
              districtDensityData.map((k, v) => MapEntry(k.toString(), v))));
  ...
  void _showEditMainDataDialog(int year) {
    ...
    final controllers = {
      'population': TextEditingController(
          text: NumberFormatUtils.formatInteger(data['population'] ?? 0)),
      'malePopulation': TextEditingController(
          text: NumberFormatUtils.formatInteger(data['malePopulation'] ?? 0)),
      'femalePopulation': TextEditingController(
          text: NumberFormatUtils.formatInteger(data['femalePopulation'] ?? 0)),
      'area': TextEditingController(
          text: NumberFormatUtils.formatDecimal(
              data['area']?.toDouble() ?? 374.0,
              decimalPlaces: 1)),
      'density': TextEditingController(
          text: NumberFormatUtils.formatInteger(data['density'] ?? 0)),
      'districts': TextEditingController(
          text: NumberFormatUtils.formatInteger(data['districts'] ?? 16)),
      'villages': TextEditingController(
          text: NumberFormatUtils.formatInteger(data['villages'] ?? 177)),
      'growthRate': TextEditingController(
          text: NumberFormatUtils.formatDecimal(
              data['growthRate']?.toDouble() ?? 0.0,
              decimalPlaces: 2)),
    };
      ...
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              _buildTextField(
                  'Total Penduduk', controllers['population']!, Icons.people,
                  isNumber: true),
              _buildTextField(
                  'Laki-laki', controllers['malePopulation']!, Icons.male,
                  isNumber: true),
              _buildTextField(
                  'Perempuan', controllers['femalePopulation']!, Icons.female,
                  isNumber: true),
              _buildTextField(
                  'Luas Area (km²)', controllers['area']!, Icons.map,
                  isDecimal: true),
              _buildTextField('Kepadatan (jiwa/km²)', controllers['density']!,
                  Icons.density_medium,
                  isNumber: true),
              _buildTextField('Jumlah Kecamatan', controllers['districts']!,
                  Icons.location_city,
                  isNumber: true),
              _buildTextField(
                  'Jumlah Kelurahan', controllers['villages']!, Icons.home_work,
                  isNumber: true),
              _buildTextField('Laju Pertumbuhan (%)',
                  controllers['growthRate']!, Icons.trending_up,
                  isDecimal: true),
            ],
          ),
        ),
          ...
          ElevatedButton(
            onPressed: () {
              try {
                setState(() {
                  pendudukData[year] = {
                    'population': NumberFormatUtils.parseIndonesianInteger(
                            controllers['population']!.text) ??
                        0,
                    'malePopulation': NumberFormatUtils.parseIndonesianInteger(
                            controllers['malePopulation']!.text) ??
                        0,
                    'femalePopulation':
                        NumberFormatUtils.parseIndonesianInteger(
                                controllers['femalePopulation']!.text) ??
                            0,
                    'area': NumberFormatUtils.parseIndonesianNumber(
                            controllers['area']!.text) ??
                        0.0,
                    'density': NumberFormatUtils.parseIndonesianInteger(
                            controllers['density']!.text) ??
                        0,
                    'districts': NumberFormatUtils.parseIndonesianInteger(
                            controllers['districts']!.text) ??
                        0,
                    'villages': NumberFormatUtils.parseIndonesianInteger(
                            controllers['villages']!.text) ??
                        0,
                    'growthRate': NumberFormatUtils.parseIndonesianNumber(
                            controllers['growthRate']!.text) ??
                        0.0,
                  };
                });
                _saveData();
  ...
  void _showEditAgeDialog(int year) {
    ...
    final controllers = {
      'usiaMuda': TextEditingController(
          text: NumberFormatUtils.formatInteger(data['usiaMuda'] ?? 0)),
      'usiaMudaPercentage': TextEditingController(
          text: NumberFormatUtils.formatDecimal(
              data['usiaMudaPercentage']?.toDouble() ?? 0.0,
              decimalPlaces: 2)),
      'usiaProduktif': TextEditingController(
          text: NumberFormatUtils.formatInteger(data['usiaProduktif'] ?? 0)),
      'usiaProduktifPercentage': TextEditingController(
          text: NumberFormatUtils.formatDecimal(
              data['usiaProduktifPercentage']?.toDouble() ?? 0.0,
              decimalPlaces: 2)),
      'usiaTua': TextEditingController(
          text: NumberFormatUtils.formatInteger(data['usiaTua'] ?? 0)),
      'usiaTuaPercentage': TextEditingController(
          text: NumberFormatUtils.formatDecimal(
              data['usiaTuaPercentage']?.toDouble() ?? 0.0,
              decimalPlaces: 2)),
    };
      ...
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Text('Usia Muda (0-14)',
                  style: TextStyle(fontWeight: FontWeight.bold)),
              _buildTextField(
                  'Jumlah', controllers['usiaMuda']!, Icons.child_care,
                  isNumber: true),
              _buildTextField('Persentase (%)',
                  controllers['usiaMudaPercentage']!, Icons.percent,
                  isDecimal: true),
              const SizedBox(height: 12),
              const Text('Usia Produktif (15-64)',
                  style: TextStyle(fontWeight: FontWeight.bold)),
              _buildTextField(
                  'Jumlah', controllers['usiaProduktif']!, Icons.work,
                  isNumber: true),
              _buildTextField('Persentase (%)',
                  controllers['usiaProduktifPercentage']!, Icons.percent,
                  isDecimal: true),
              const SizedBox(height: 12),
              const Text('Usia Tua (65+)',
                  style: TextStyle(fontWeight: FontWeight.bold)),
              _buildTextField('Jumlah', controllers['usiaTua']!, Icons.elderly,
                  isNumber: true),
              _buildTextField('Persentase (%)',
                  controllers['usiaTuaPercentage']!, Icons.percent,
                  isDecimal: true),
            ],
          ),
        ),
            ...
            onPressed: () {
              try {
                final total = (NumberFormatUtils.parseIndonesianNumber(
                            controllers['usiaMudaPercentage']!.text) ??
                        0.0) +
                    (NumberFormatUtils.parseIndonesianNumber(
                            controllers['usiaProduktifPercentage']!.text) ??
                        0.0) +
                    (NumberFormatUtils.parseIndonesianNumber(
                            controllers['usiaTuaPercentage']!.text) ??
                        0.0);
                ...
                setState(() {
                  ageDistributionData[year] = {
                    'usiaMuda': NumberFormatUtils.parseIndonesianInteger(
                            controllers['usiaMuda']!.text) ??
                        0,
                    'usiaMudaPercentage':
                        NumberFormatUtils.parseIndonesianNumber(
                                controllers['usiaMudaPercentage']!.text) ??
                            0.0,
                    'usiaProduktif': NumberFormatUtils.parseIndonesianInteger(
                            controllers['usiaProduktif']!.text) ??
                        0,
                    'usiaProduktifPercentage':
                        NumberFormatUtils.parseIndonesianNumber(
                                controllers['usiaProduktifPercentage']!.text) ??
                            0.0,
                    'usiaTua': NumberFormatUtils.parseIndonesianInteger(
                            controllers['usiaTua']!.text) ??
                        0,
                    'usiaTuaPercentage':
                        NumberFormatUtils.parseIndonesianNumber(
                                controllers['usiaTuaPercentage']!.text) ??
  ...
  void _showEditDistrictDialog(int year, int index) {
    ...
    final controllers = {
      ...
      'density': TextEditingController(
          text: NumberFormatUtils.formatInteger(district['density'] ?? 0)),
      'population': TextEditingController(
          text: NumberFormatUtils.formatDecimal(
              district['population']?.toDouble() ?? 0.0,
              decimalPlaces: 3)),
    };
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            _buildTextField(
                'Nama Kecamatan', controllers['name']!, Icons.location_city),
            _buildTextField(
                'Kepadatan', controllers['density']!, Icons.density_medium,
                isNumber: true),
            _buildTextField(
                'Populasi (ribu)', controllers['population']!, Icons.people,
                isDecimal: true),
          ],
        ),
        actions: [
                ...
                setState(() {
                  districtDensityData[year]![index] = {
                    ...
                    'density': NumberFormatUtils.parseIndonesianInteger(
                            controllers['density']!.text) ??
                        0,
                    'population': NumberFormatUtils.parseIndonesianNumber(
                            controllers['population']!.text) ??
                        0.0,
                  };
                });
    ...
    final mainControllers = {
      'population': TextEditingController(
          text: NumberFormatUtils.formatInteger(
              (lastData['population'] * growth).round())),
      'malePopulation': TextEditingController(
          text: NumberFormatUtils.formatInteger(
              (lastData['malePopulation'] * growth).round())),
      'femalePopulation': TextEditingController(
          text: NumberFormatUtils.formatInteger(
              (lastData['femalePopulation'] * growth).round())),
      'area': TextEditingController(
          text: NumberFormatUtils.formatDecimal(
              lastData['area']?.toDouble() ?? 374.0,
              decimalPlaces: 1)),
      'density': TextEditingController(
          text: NumberFormatUtils.formatInteger(
              (lastData['density'] * growth).round())),
      'districts': TextEditingController(
          text: NumberFormatUtils.formatInteger(lastData['districts'] ?? 16)),
      'villages': TextEditingController(
          text: NumberFormatUtils.formatInteger(lastData['villages'] ?? 177)),
      'growthRate': TextEditingController(
          text: NumberFormatUtils.formatDecimal(0.5, decimalPlaces: 2)),
    };

    final ageControllers = {
      'usiaMuda': TextEditingController(
          text: NumberFormatUtils.formatInteger(
              ((lastAge['usiaMuda'] ?? 0) * growth).round())),
      'usiaMudaPercentage': TextEditingController(
          text: NumberFormatUtils.formatDecimal(
              (lastAge['usiaMudaPercentage'] ?? 20.0).toDouble(),
              decimalPlaces: 2)),
      'usiaProduktif': TextEditingController(
          text: NumberFormatUtils.formatInteger(
              ((lastAge['usiaProduktif'] ?? 0) * growth).round())),
      'usiaProduktifPercentage': TextEditingController(
          text: NumberFormatUtils.formatDecimal(
              (lastAge['usiaProduktifPercentage'] ?? 70.0).toDouble(),
              decimalPlaces: 2)),
      'usiaTua': TextEditingController(
          text: NumberFormatUtils.formatInteger(
              ((lastAge['usiaTua'] ?? 0) * growth).round())),
      'usiaTuaPercentage': TextEditingController(
          text: NumberFormatUtils.formatDecimal(
              (lastAge['usiaTuaPercentage'] ?? 10.0).toDouble(),
              decimalPlaces: 2)),
    };
    ...
    final districtControllers = List.generate(5, (i) {
      if (i < lastDistricts.length) {
        return {
          ...
          'density': TextEditingController(
              text: NumberFormatUtils.formatInteger(
                  (lastDistricts[i]['density'] * growth).round())),
          'population': TextEditingController(
              text: NumberFormatUtils.formatDecimal(
                  (lastDistricts[i]['population'] * growth).toDouble(),
                  decimalPlaces: 3)),
        };
      }
      return {
        ...
        'density':
            TextEditingController(text: NumberFormatUtils.formatInteger(4500)),
        'population': TextEditingController(
            text: NumberFormatUtils.formatDecimal(150.0, decimalPlaces: 3)),
      };
    });
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const Text('DATA UTAMA',
                    style:
                        TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(height: 8),
                _buildTextField(
                    'Populasi', mainControllers['population']!, Icons.people,
                    isNumber: true),
                _buildTextField(
                    'Laki-laki', mainControllers['malePopulation']!, Icons.male,
                    isNumber: true),
                _buildTextField('Perempuan',
                    mainControllers['femalePopulation']!, Icons.female,
                    isNumber: true),
                _buildTextField(
                    'Area (km²)', mainControllers['area']!, Icons.map,
                    isDecimal: true),
                _buildTextField('Kepadatan', mainControllers['density']!,
                    Icons.density_medium,
                    isNumber: true),
                _buildTextField('Kecamatan', mainControllers['districts']!,
                    Icons.location_city,
                    isNumber: true),
                _buildTextField(
                    'Kelurahan', mainControllers['villages']!, Icons.home_work,
                    isNumber: true),
                _buildTextField('Growth Rate (%)',
                    mainControllers['growthRate']!, Icons.trending_up,
                    isDecimal: true),
                const SizedBox(height: 16),
                const Text('DISTRIBUSI UMUR',
                    style:
                        TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(height: 8),
                _buildTextField(
                    'Usia Muda', ageControllers['usiaMuda']!, Icons.child_care,
                    isNumber: true),
                _buildTextField('Usia Muda %',
                    ageControllers['usiaMudaPercentage']!, Icons.percent,
                    isDecimal: true),
                _buildTextField('Usia Produktif',
                    ageControllers['usiaProduktif']!, Icons.work,
                    isNumber: true),
                _buildTextField('Usia Produktif %',
                    ageControllers['usiaProduktifPercentage']!, Icons.percent,
                    isDecimal: true),
                _buildTextField(
                    'Usia Tua', ageControllers['usiaTua']!, Icons.elderly,
                    isNumber: true),
                _buildTextField('Usia Tua %',
                    ageControllers['usiaTuaPercentage']!, Icons.percent,
                    isDecimal: true),
                const SizedBox(height: 16),
                const Text('5 KECAMATAN TERPADAT',
                    style:
                        TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(height: 8),
                ...List.generate(
                    ...
                    (i) => Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _buildTextField(
                                  'Nama',
                                  districtControllers[i]['name']!,
                                  Icons.location_city),
                              _buildTextField(
                                  'Kepadatan',
                                  districtControllers[i]['density']!,
                                  Icons.density_medium,
                                  isNumber: true),
                              _buildTextField(
                                  'Populasi (ribu)',
                                  districtControllers[i]['population']!,
                                  Icons.people,
                                  isDecimal: true),
          ...
          ElevatedButton(
            onPressed: () {
              try {
                ...
                final totalPercentage = (NumberFormatUtils
                            .parseIndonesianNumber(
                                ageControllers['usiaMudaPercentage']!.text) ??
                        0.0) +
                    (NumberFormatUtils.parseIndonesianNumber(
                            ageControllers['usiaProduktifPercentage']!.text) ??
                        0.0) +
                    (NumberFormatUtils.parseIndonesianNumber(
                            ageControllers['usiaTuaPercentage']!.text) ??
                        0.0);
                ...
                setState(() {
                  pendudukData[nextYear] = {
                    'population': NumberFormatUtils.parseIndonesianInteger(
                            mainControllers['population']!.text) ??
                        0,
                    'malePopulation': NumberFormatUtils.parseIndonesianInteger(
                            mainControllers['malePopulation']!.text) ??
                        0,
                    'femalePopulation':
                        NumberFormatUtils.parseIndonesianInteger(
                                mainControllers['femalePopulation']!.text) ??
                            0,
                    'area': NumberFormatUtils.parseIndonesianNumber(
                            mainControllers['area']!.text) ??
                        0.0,
                    'density': NumberFormatUtils.parseIndonesianInteger(
                            mainControllers['density']!.text) ??
                        0,
                    'districts': NumberFormatUtils.parseIndonesianInteger(
                            mainControllers['districts']!.text) ??
                        0,
                    'villages': NumberFormatUtils.parseIndonesianInteger(
                            mainControllers['villages']!.text) ??
                        0,
                    'growthRate': NumberFormatUtils.parseIndonesianNumber(
                            mainControllers['growthRate']!.text) ??
                        0.0,
                  };

                  ageDistributionData[nextYear] = {
                    'usiaMuda': NumberFormatUtils.parseIndonesianInteger(
                            ageControllers['usiaMuda']!.text) ??
                        0,
                    'usiaMudaPercentage':
                        NumberFormatUtils.parseIndonesianNumber(
                                ageControllers['usiaMudaPercentage']!.text) ??
                            0.0,
                    'usiaProduktif': NumberFormatUtils.parseIndonesianInteger(
                            ageControllers['usiaProduktif']!.text) ??
                        0,
                    'usiaProduktifPercentage':
                        NumberFormatUtils.parseIndonesianNumber(
                                ageControllers['usiaProduktifPercentage']!
                                    .text) ??
                            0.0,
                    'usiaTua': NumberFormatUtils.parseIndonesianInteger(
                            ageControllers['usiaTua']!.text) ??
                        0,
                    'usiaTuaPercentage':
                        NumberFormatUtils.parseIndonesianNumber(
                                ageControllers['usiaTuaPercentage']!.text) ??
                  ...
                  districtDensityData[nextYear] = List.generate(
                      5,
                      (i) => {
                            'name': districtControllers[i]['name']!.text,
                            'density': NumberFormatUtils.parseIndonesianInteger(
                                    districtControllers[i]['density']!.text) ??
                                0,
                            'population':
                                NumberFormatUtils.parseIndonesianNumber(
                                        districtControllers[i]['population']!
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildDataRow(
                    'Populasi',
                    _formatNumber(pendudukData[selectedYear]!['population']),
                    Icons.people),
                _buildDataRow(
                    'Laki-laki',
                    _formatNumber(
                        pendudukData[selectedYear]!['malePopulation']),
                    Icons.male),
                _buildDataRow(
                    'Perempuan',
                    _formatNumber(
                        pendudukData[selectedYear]!['femalePopulation']),
                    Icons.female),
                _buildDataRow('Area',
                    '${pendudukData[selectedYear]!['area']} km²', Icons.map),
                _buildDataRow(
                    'Kepadatan',
                    '${_formatNumber(pendudukData[selectedYear]!['density'])} jiwa/km²',
                    Icons.density_medium),
                _buildDataRow(
                    'Kecamatan',
                    '${pendudukData[selectedYear]!['districts']}',
                    Icons.location_city),
                _buildDataRow(
                    'Kelurahan',
                    '${pendudukData[selectedYear]!['villages']}',
                    Icons.home_work),
                _buildDataRow(
                    'Growth Rate',
                    '${pendudukData[selectedYear]!['growthRate']}%',
                    Icons.trending_up),
              ],
            ),
          ),
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                      'Edit data distribusi umur untuk grafik pie chart',
          ...
          return Card(
            ...
            child: ListTile(
              ...
              subtitle: Padding(
                ...
                child: Text(
                    'Muda: ${data['usiaMudaPercentage']}% | Produktif: ${data['usiaProduktifPercentage']}% | Tua: ${data['usiaTuaPercentage']}%',
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                SizedBox(width: 12),
                Expanded(
                  child: Text('Edit data 5 kecamatan terpadat',
                      style: TextStyle(fontSize: 14)),
                ),
              ],
            ),
        ...
        ...availableYears.map((year) {
          ...
          return Card(
            ...
            child: ExpansionTile(
              ...
              title: Text('Tahun $year',
                  style: TextStyle(fontWeight: FontWeight.w600)),
              subtitle: Text('${districts.length} kecamatan',
                  style: TextStyle(fontSize: 12)),
              children: List.generate(districts.length, (i) {
                ...
                return ListTile(
                  ...
                  subtitle: Text(
                      'Density: ${d['density']} | Pop: ${d['population']} ribu',
                      style: TextStyle(fontSize: 12)),
                  trailing: IconButton(
        ...
        ...availableYears.map((year) => Card(
              ...
              child: ListTile(
                ...
                title: Text('Tahun $year',
                    style: TextStyle(fontWeight: FontWeight.w600)),
                subtitle: Text(
                    'Populasi: ${_formatNumber(pendudukData[year]!['population'])}',
                    style: TextStyle(fontSize: 12)),
                trailing: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                      ...
                      actions: [
                        ...
                        ElevatedButton(
                          onPressed: () {
                            ...
                            _initializeDefaultDistrictData();
                            setState(() {
                              availableYears = pendudukData.keys.toList()
                                ..sort();
                              selectedYear = availableYears.last;
                            });
                            _saveData();
                    ...
                    builder: (context) => AlertDialog(
                      ...
                      content: const SingleChildScrollView(
                        child: Column(
                          ...
                          children: [
                            Text(
                                'Admin Panel untuk mengelola data penduduk Kota Semarang.',
                                style: TextStyle(fontSize: 14)),
                            SizedBox(height: 16),
                            Text('Fitur:',
                                style: TextStyle(
